...
- **★ Local-feature matching (ORB + RANSAC) — best measured descriptor, 2026-08-04** [L, High] — on the same 8000-ref pool and all 209 real warps: **98.6% top-1 / 99.0% top-5**, vs DINOv2 art+auto 80% and pHash 59%. Fixes **every** card, including Strategic Betrayal (58/61 = 95%) which neither other descriptor could (16% / 6%). Local gradient descriptors ignore global statistics, so the photometric domain gap doesn't apply; RANSAC homography verification is qualitatively stronger evidence than a distance threshold. Recipe: art crop → gray → 480 px → `equalizeHist` → ORB(400) → BFMatcher Hamming knn k=2 + Lowe 0.75 → top-25 shortlist → `findHomography(RANSAC,5.0)` → rank by inliers. **Supersedes the embedding as the recommended direction** (no model, no TFLite; OpenCV already ships via `dartcv4`). **Deployment is the open problem, not accuracy:** brute force is 7.3 s/query at 8k refs ⇒ **~46 s at 49,877** (needs a BoW/VLAD inverted index), and raw descriptors are 12.8 KB/card ⇒ **~640 MB** vs the 88 MB bundle (needs a compact index; which in turn complicates RANSAC, as verification needs real keypoints). Harness: `dataprep/bench/eval_local.py`.
  ...
  - **★ FULL-SCALE VALIDATION PASSED (2026-08-18, `eval_fullscale.py`) — the design holds at production scale.** Re-ran the same method against ALL **49,877** cards (previous figures all used an 8,000 pool): **count-only 94.3%** (8k: 94.7%) and **+RANSAC 95.7% — identical to the 8k result**. Per card: flare **33/33**, force **34/34**, subtlety **32/32**, strategic 55/61 count / **58/61** ransac, dauthi 25/27 (was 27/27), undercity 18/22 (was 22/22). Brute force measured **7.8 s/query**, matching the projection. Two consequences: (a) **RANSAC's value GROWS with scale** (+1.4 pp here vs ~1 pp at 8k — more distractors produce more spurious match-counts and geometric verification filters them) ⇒ **`orb_desc` should ship**, it is load-bearing, not marginal; (b) the small losses landed on the EASY cards (dauthi, undercity) while the three hard cards stayed perfect — which is exactly the hybrid's premise, since pHash scores 100% on those and routing means they never reach ORB.
  - **★ SPARSE INVERTED INDEX (2026-08-18, `eval_invindex.py`) — retrieval solved.** Dense `R @ q` **72.2 ms** → CSC column-slice **0.5 ms/query**, **144× faster**, with scores identical (max diff 1.79e-07 — asserted, so it is an optimization and not a different-answers-faster bug). **Projected at 50k: 450 ms → ~3 ms.** *Correction to the earlier explanation:* the win is NOT "touching fewer cards" (99% of cards are still touched) — it is sparsity in the WORD dimension: each card activates only **271 of 65,536 words (0.4%)**, so the slice walks 2.17M postings instead of 524M dense entries. ⚠️ **This promotes QUANTIZATION to the bottleneck** (flat-vocab ~200 ms vs retrieval ~3 ms and sub-ms re-rank), so the earlier "the tree is an optimization, not a prerequisite" is **superseded — fast quantization is now on the critical path.** Alternative worth trying before more tree debugging: **binary centroids + Hamming** instead of float Euclidean (natural for ORB, far cheaper than 1.7 GFLOP). ⚠️ **Schema correction:** the index is **271 words/card**, not ~100 (soft-3 triples it) ⇒ full CSC ≈ **109 MB at 50k**, not the ~23 MB estimated for `orb_bow` above; storing word IDs without weights would roughly halve that.
