...
- **OCR-path speedups for hard cards** [M, Med–High] — until the deep fix lands, the noise-floor cards above always hit the OCR tiebreak, which is slow: ML Kit OCR ~800–1000 ms warm (**~2000 ms on the FIRST call — one-time model warm-up**), a redundant second detect+warp for the title JPEG, and it runs **2× for consensus** (+ retries on glare misreads like `"fore of Negatlon"`, `"Bubtlety"`). Candidate proposals: **P-D** pre-warm ML Kit at startup (~1 s off the first hard card, bit-neutral); **P-B** reuse the `full`-pass warp for the JPEG instead of re-detecting (~100 ms/frame, bit-neutral); **P-A** commit on the **first** OCR-positively-confirmed frame instead of requiring 2 (halves OCR passes; behavioral → proposal); **P-C** fuzzy name match to rescue glare misreads (accuracy-sensitive). Est. Force of Negation / Strategic Betrayal ~3 s → ~1.2 s.
